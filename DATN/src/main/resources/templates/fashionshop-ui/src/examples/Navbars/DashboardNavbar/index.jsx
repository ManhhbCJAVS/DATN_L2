...
function DashboardNavbar({ absolute, light, isMini }) {
  ...
  // Lấy employeeCode nếu đang ở trang chi tiết nhân viên
  let employeeCode = null;
  if (route[0] === "NhanVien" && route.length === 2) {
    employeeCode = localStorage.getItem("currentEmployeeCode");
  }
      ...
      <Toolbar sx={(theme) => navbarContainer(theme)}>
        ...
        >
          ...
          <IconButton
            size="large" // to hơn
            color="inherit"
            sx={{ ...navbarMobileMenu, mr: 2 }} // mr:2 = marginRight: theme.spacing(2)
            onClick={handleMiniSidenav}
          >
            <Icon
              className={light ? "text-white" : "text-dark"}
              fontSize="large" // to hơn
            >
          ...
          <Breadcrumbs
            icon="home"
            title={
              route[0] === "NhanVien" && route.length === 2 && employeeCode
                ? employeeCode
